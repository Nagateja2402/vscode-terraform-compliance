...
const codeActionProvider_1 = require("../codeActionProvider/codeActionProvider");
class SuggestionProvider {
    ...
    lastAnalyzedContent = new Map();
    isAnalyzing = false;
    debounceDelay = 2000; // Wait 2 seconds after user stops typing
    periodicAnalysisInterval = 30000; // Check every 30 seconds
    periodicTimer;
    statusBarItem;
    debugLogging = false;
    maxSuggestionsPerFile = 5; // Prevent UI overcrowding
    constructor(llmService, context) {
        ...
        this.codeActionProvider = vscode.languages.registerCodeActionsProvider({ language: 'terraform' }, new codeActionProvider_1.ComplianceCodeActionProvider(this), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] });
    ...
    scheduleAnalysis(document, customDelay) {
        if (!this.autoAnalysisEnabled)
            return;
    ...
    async analyzeDocument(document) {
        if (!this.autoAnalysisEnabled || this.isAnalyzing)
            return;
    ...
    processSuggestions(document, suggestions) {
    ...
    removeOverlappingSuggestions(suggestions) {
    ...
    findCodeLocation(document, suggestion) {
        ...
        if (exactMatch.found) {
            return {
                ...
                range: new vscode.Range(exactMatch.startLine, 0, exactMatch.endLine, lines[exactMatch.endLine]?.length || 0),
                found: true
            };
        }
        ...
        if (resourceMatch.found) {
            return {
                ...
                range: new vscode.Range(resourceMatch.startLine, 0, resourceMatch.endLine, lines[resourceMatch.endLine]?.length || 0),
                found: true
            };
        }
        ...
        if (fuzzyMatch.found) {
            return {
                ...
                range: new vscode.Range(fuzzyMatch.startLine, 0, fuzzyMatch.endLine, lines[fuzzyMatch.endLine]?.length || 0),
                found: true
            };
        }
    ...
    findExactMultiLineMatch(lines, originalCode) {
    ...
    findTerraformResourceMatch(lines, originalCode) {
    ...
    findFuzzyMatch(lines, originalCode) {
        const originalLines = originalCode.split('\n').map(line => line.trim()).filter(line => line.length > 0);
    ...
    findInsertionPoint(document, suggestion) {
    ...
    extractTerraformResourceInfo(code) {
    ...
    createHoverMessage(suggestion) {
        ...
        message.appendMarkdown(`[Accept](${acceptUri} "Apply this suggestion") • ` +
    ...
    async applySuggestion(suggestion) {
        ...
        let suggestionWithLocation;
        if ('_location' in suggestion) {
            suggestionWithLocation = suggestion;
        }
        else {
            ...
            const found = suggestions.find(s => s.original_code_snippet === suggestion.original_code_snippet &&
    ...
    getProperIndentation(document, lineNumber) {
    ...
    removeSuggestion(uri, suggestion) {
        ...
        const index = suggestions.findIndex(s => s.original_code_snippet === suggestion.original_code_snippet &&
            s.suggested_code_snippet === suggestion.suggested_code_snippet &&
            s.line_number === suggestion.line_number &&
            s.reasoning === suggestion.reasoning);
        if (index > -1) {
            ...
            if (document && suggestions.length > 0) {
                ...
                const basicSuggestions = suggestions.map(s => ({
    ...
    async manualAnalyzeDocument(document) {
        ...
        try {
            ...
            if (suggestionCount > 0) {
                ...
                vscode.window.showInformationMessage(`Found ${suggestionCount} compliance ${suggestionCount === 1 ? 'issue' : 'issues'}. ` +
